 ...
 */
/******************************************************************/
/* $Id: ecc.h,v 1.19 2021/07/22 08:28:23 cooper Exp $ */
...
#define NCB_NoDevice             9              /**< No device found                       */
#define NCB_InvalidParam        11              /**< Parameter out of range                */
#define NCB_FeatureNotAvailable 12              /**< Feature only available in pro version */
...
/** @brief Check devices
 *
 *  Discovers devices that are connected by USB or Ethernet and returns the number
 *  of discovered devices.
 *  The function returns a pointer to an array of struct @ref EccInfo. The return code
 *  is the number of found devices (and the number of valid elements in the array).
 ...
 *
 *  The function has to be called before connecting to a device by @ref ECC_Connect.
 *  It must not be called as long as a device is connected.
 *  
 *  @param  info  Output: Pointer to array of struct 'EccInfo'. The parameter may be NULL
 *                to ignore the devices array (see @ref ECC_getDeviceInfo).
 *  @return Number of found devices
 */
Int32 NCB_API ECC_Check( struct EccInfo** info );
 ...
 *
 *  @ref ECC_Check() is able to find devices connected via TCP/IP
 ...
 *  calling @ref ECC_Check. The function registers one device and can
 *  be called several times.
 *  Registered and reachable devices will be found by @ref ECC_Check.
 *  @param    hostname  Hostname or IP Address in dotted decimal notation
 *                      of the device to register.
 *  @return             Error code. NCB_NoDevice means here that the
 *                      hostname could not be resolved. A return code of NCB_Ok
 *                      doesn't guarantee that the device is reachable.
 */
Int32 NCB_API ECC_registerExternalIp( const char * hostname );



/** @brief Connect device
 *
 *  Initializes and connects the selected ECC device.
 *  The function has to be called before any function operating the device,
 *  but after @ref ECC_Check.
 *
 *  @param  deviceNo:     Index of the device to connect, 
